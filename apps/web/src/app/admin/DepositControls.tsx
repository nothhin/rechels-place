...
export function DepositControls({ bookingId, bookingStatus = "pending", depositStatus, canManage }: { bookingId: string; bookingStatus?: string; depositStatus: string; canManage: boolean }) {
  ...
  return <div className="deposit-admin-actions">
    <div className="deposit-workflow-header"><span>Payment workflow</span><strong>{depositStatus === "submitted" ? "Verify submitted deposit" : depositStatus === "verified" || depositStatus === "refund_pending" ? "Deposit verified" : "Record payment & confirm"}</strong><small>Verify the payment account before confirming the stay. Record any remaining balance separately.</small></div>
