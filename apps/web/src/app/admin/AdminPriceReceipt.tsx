...
export function AdminPriceReceipt({ booking }: { booking:AdminEnquiry }) {
  ...
  const depositAmountConfirmed = ["verified", "refund_pending", "refunded", "partially_withheld", "forfeited"].includes(booking.depositStatus);
  return <section className={styles.adminReceipt} aria-label="Booking payment receipt"><header><strong>Payment summary</strong><span>{booking.remainingBalanceMinor === 0 ? "Fully paid" : "Balance due"}</span></header><dl>
    ...
    <div><dt>Reference nightly rate</dt><dd>{php.format(booking.baseNightlyRateMinor/100)}</dd></div>
    ...
    <div><dt>Refundable security deposit</dt><dd>{depositAmountConfirmed ? php.format(booking.depositAmountMinor/100) : "Host confirms"}</dd></div>
